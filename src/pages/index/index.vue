...
<template>
  <view class="page safe-top home">
    <view class="topline"><view><text class="eyebrow">FEEFLOW / 2026</text><text class="title">预算清晰，<br /><text class="accent">经营更从容</text></text></view><view class="avatar">许</view></view>
    <view class="hero card"><view><text class="hero-kicker">今日费控概览</text><text class="hero-title">每一笔费用，<br />都应该花得明白</text><text class="hero-note">费用申请 · 发票归集 · 预算审批<br />小团队也能用好的免费费控系统</text></view><text class="hero-mark">¥</text></view>
    <view v-if="loading" class="state card"><text class="state-icon">◌</text>正在同步预算数据…</view>
    <template v-else>
      <view class="metric-grid"><view class="metric card"><text class="metric-label">本月报销</text><text class="metric-value">{{ money(dashboard.todaySales) }}</text><text class="metric-trend">↗ 较昨日 +12.8%</text></view><view class="metric card"><text class="metric-label">本月预算</text><text class="metric-value">{{ money(dashboard.stockValue) }}</text><text class="metric-trend blue">{{ warehouses.length }} 个成本中心</text></view></view>
      <view class="quick-grid"><view class="quick card" @click="uni.navigateTo({ url: '/pages/workbench/workbench' })"><text class="quick-icon orange">＋</text><text class="quick-name">快速入库</text><text class="small muted">登记报销到货</text></view><view class="quick card" @click="uni.navigateTo({ url: '/pages/orders/orders' })"><text class="quick-icon">↗</text><text class="quick-name">付款审批</text><text class="small muted">处理 {{ dashboard.pendingOutbound }} 笔待付款</text></view></view>
      <view class="section-head"><text class="section-title">预算提醒</text><text class="section-more">{{ dashboard.lowStock }} 项需要补货</text></view>
