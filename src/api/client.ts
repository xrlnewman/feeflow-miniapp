...
const demoProducts: ProductItem[] = [
  { id: 'p-1001', sku: 'FE-1001', name: '轻量通勤双肩包', category: '箱包', warehouse: '东城中心仓', unit: '件', stock: 126, minStock: 40, price: 169, status: 'normal' },
  { id: 'p-1002', sku: 'FE-1002', name: '便携保温杯 480ml', category: '家居', warehouse: '东城中心仓', unit: '件', stock: 18, minStock: 30, price: 89, status: 'warning' },
  { id: 'p-1003', sku: 'FE-1003', name: '有机棉基础 T 恤', category: '服饰', warehouse: '西郊备货仓', unit: '件', stock: 9, minStock: 24, price: 129, status: 'danger' },
  { id: 'p-1004', sku: 'FE-1004', name: '桌面收纳盒套装', category: '家居', warehouse: '西郊备货仓', unit: '套', stock: 64, minStock: 20, price: 59, status: 'normal' },
]
...
const demoPurchases: PurchaseOrder[] = [{ id: 'PO20260716001', supplier: '清和供应链', warehouse: '东城中心仓', items: 3, amount: 12800, status: '待审批', createdAt: '2026-07-16T08:30:00Z' }, { id: 'PO20260715008', supplier: '织物工坊', warehouse: '西郊备货仓', items: 2, amount: 7360, status: '已入库', createdAt: '2026-07-15T08:30:00Z' }]
const demoSales: SalesOrder[] = [{ id: 'SO20260716032', customer: '星河生活馆', warehouse: '东城中心仓', items: 8, amount: 3280, status: '待付款', createdAt: '2026-07-16T09:12:00Z' }, { id: 'SO20260716031', customer: '林先生', warehouse: '西郊备货仓', items: 2, amount: 258, status: '已完成', createdAt: '2026-07-16T08:40:00Z' }]
...
function readToken(): string | undefined {
  ...
  try { const uni = runtime.uni?.getStorageSync?.('feeflow_access_token'); if (typeof uni === 'string' && uni) return uni; return runtime.localStorage?.getItem('feeflow_access_token') || undefined } catch { return undefined }
}
...
export function createApiClient(options: ApiClientOptions = {}): ApiClient {
  ...
  async function fallback<T>(name: string, value: T, action: () => Promise<T>): Promise<T> { if (!baseUrl) return clone(value); try { return await action() } catch (error) { console.warn(`[FeeFlow API] ${name} 请求失败，已回退演示数据`, error); return clone(value) } }
  return {
    ...
    receivePurchase: (id) => fallback('purchase-receive', demoPurchases.find((item) => item.id === id) ?? demoPurchases[0], () => remote<PurchaseOrder>(`/purchase-orders/${id}/receive`, 'POST', {}, { 'Idempotency-Key': `feeflow-purchase-${id}` })),
    fetchSalesOrders: () => fallback('sales-orders', demoSales, async () => page(await remote<{ list?: SalesOrder[] }>('/sales-orders', 'GET'))),
    shipSale: (id) => fallback('sales-ship', demoSales.find((item) => item.id === id) ?? demoSales[0], () => remote<SalesOrder>(`/sales-orders/${id}/ship`, 'POST', {}, { 'Idempotency-Key': `feeflow-sale-${id}` })),
